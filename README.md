...
一个适合应届生简历展示的 Java 单体电商项目。它刻意保持了合理的业务完整度和较低的部署门槛：无需安装前端依赖或数据库，克隆后执行一条 Maven 命令即可体验商品浏览、购物车、优惠结算、订单流转和运营后台。

## 项目定位

- 不是微服务练习场：不为了技术名词引入 Redis、MQ、网关或注册中心。
- 是完整业务闭环：用户下单后可模拟支付，管理员可发货，用户可确认收货或取消订单。
- 亮点可验证：防超卖、幂等下单和订单状态机均有实际代码与自动化测试。
...
- 订单：下单、价格快照、优惠快照、模拟支付、取消、发货、确认收货。
...
更详细的接口表见 [`docs/API.md`](docs/API.md)，简历描述和面试讲法见 [`docs/INTERVIEW.md`](docs/INTERVIEW.md)。

## 取舍说明

这是一个简历项目，因此支付使用本地模拟，文件图片使用 CSS 与字符图标，无真实短信、物流和第三方支付。CSRF 在当前同源演示应用中关闭；如果上线，应启用 CSRF Token、验证码、限流、审计日志和数据库迁移工具。这样的边界比“假装已经生产可用”更容易在面试中讲清楚。
